...
from bug_fixer.repo_manager import (
    clone_to_temp,
    cleanup,
...
def build_pr_body(result: dict, issue_number: int) -> str:
    """Construct the PR description from a successful fix result."""
    lines = [
        ...
        "",
        "## Test results",
        f"- Before: {result['pre_pass_count']} passing, {result['pre_fail_count']} failing",
        f"- After: {result['post_pass_count']} passing, {result['post_fail_count']} failing",
        "",
        "### Newly passing tests",
    ]
    lines.extend(f"- `{t}`" for t in result["newly_passing"])
    if result["still_failing"]:
        lines.append("")
        lines.append("### Still failing (unrelated to this fix)")
        lines.extend(f"- `{t}`" for t in result["still_failing"])
    lines.extend([
        "",
        "---",
        "🤖 Generated by [bug-fixer](https://github.com/timuryesm/bug-fixer).",
    ])
    return "\n".join(lines)
...
def run_fix(
    ...
    if missing_tests:
        ...
        print("\n--- Test output ---")
        print(post_output)
        return {"success": False, "reason": "catastrophic"}
    elif new_failures:
        ...
        return {
            ...
            "still_failing": sorted(post_fail),
            "pre_pass_count": len(pre_pass),
            "pre_fail_count": len(pre_fail),
            "post_pass_count": len(post_pass),
            "post_fail_count": len(post_fail),
        }
...
def main() -> None:
    ...
    try:
        ...
        if args.open_pr and result["success"] and issue is not None:
            if not github_token:
                sys.exit("--open-pr requires GITHUB_TOKEN to be set.")

            branch_name = f"bug-fixer/issue-{issue.number}"
            commit_message = f"Fix #{issue.number}: {issue.title}"

            print(f"\n🌿 Creating branch '{branch_name}' and committing fix ...")
            create_branch_and_commit(repo_path, branch_name, commit_message)

            print(f"📤 Pushing branch to {issue.owner}/{issue.repo} ...")
            push_branch(repo_path, branch_name, issue.owner, issue.repo, github_token)

            base = get_default_branch(issue.owner, issue.repo, github_token)
            print(f"📬 Opening PR against base branch '{base}' ...")
            pr = open_pull_request(
                owner=issue.owner,
                repo=issue.repo,
                title=commit_message,
                body=build_pr_body(result, issue.number),
                head=branch_name,
                base=base,
                token=github_token,
            )
